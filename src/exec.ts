...
export class RunApp {
    ...
    private handleLog = (logEntry: string) => {
        ...
        if ((this.wrapConfig.auto_recover || false) && (/ERR.*CONSENSUS.*FAILURE/.test(logEntry) || /panic.*precommit.*step.*prevoted.*for.*an.*invalid.*block.*wrong.*Block.Header.AppHash/.test(logEntry))) {
